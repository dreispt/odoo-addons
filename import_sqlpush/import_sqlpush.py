...
class import_sqlpush(orm.Model):
    ...
    def do_import_data(self, cr, uid, ids=None, context=None):
        ...
        'data' should use \n - char(10) as line separator and \t - char(9)
        as column separator.

        PSQL EXAMPLE:
        INSERT INTO import_sqlpush (model, data)
        VALUES ('res.partner', E'id\tref\tname\tcomment\nres_partner_PUSHME\tPUSHME\tPushed Customer\tThis is a test')
        """
        ...
        for r in self.browse(cr, uid, ids, context=context):
            try:
                ...
                assert len(lines) > 1, \
                    "Data must have at least a header and a data row"
                assert len(lines[0]) == len(lines[1]), \
                    "Header and rows must have same number of cols"
                ...
                if r.options and 'unlink' in r.options:
                    unlink_ids = model_data.search(cr, uid,
                        [('model', '=', r.model), ('name', '=', r.key_ref)])
                    if unlink_ids:
                        model.unlink(cr, uid, unlink_ids, context=context)

                log = "try import"
                res = model.import_data(cr, uid, lines[0], lines[1:],
                    noupdate=noupdate_flag)
                #print r.key_ref, noupdate_flag, res
                if res[0] < 0:
                    errs += 1
                    self.write(cr, uid, [r.id],
                      {'state': 'cancel', 'log': 'Import error %s' % (res[2])})
                else:
                    ok_ids.append(r.id)
        ...
        if ok_ids:
            end_of_batch(ok_ids)
        if errs:
            _logger.warn('%s errors found. See the SQLPush table for details.'
                         % errs)
        return True

    def do_purge(self, cr, uid, ids=None, lag=7, context=None):
        """Delete executed records older than 'lag' days"""
        cr.execute("DELETE FROM import_sqlpush WHERE state<>'draft'"
